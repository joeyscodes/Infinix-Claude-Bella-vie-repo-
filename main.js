...
(function () {
  ...
  /* ---------------------------------------------------------
     10. BOOKING FORM — Formspree (primary, email) + WhatsApp (secondary)
     --------------------------------------------------------- */
  ...
  if (bookingForm) {
    bookingForm.addEventListener('submit', async (e) => {
      ...
      const isPlaceholder = bookingForm.action.includes('YOUR_FORM_ID');

      if (isPlaceholder) {
        // Formspree not yet configured — fall back to WhatsApp so the booking still goes through.
        if (bookingStatus) {
          bookingStatus.textContent = 'Booking form isn\u2019t fully connected yet — sending your request via WhatsApp instead.';
      ...
      if (submitBtn) { submitBtn.disabled = true; submitBtn.querySelector('.btn-label').textContent = 'Sending...'; }
      if (bookingStatus) { bookingStatus.textContent = ''; bookingStatus.className = 'form-status'; }
      ...
      try {
        ...
        if (res.ok) {
          bookingForm.reset();
          if (bookingStatus) {
            bookingStatus.textContent = 'Thank you — your booking request has been sent. We\u2019ll confirm shortly.';
        ...
        if (bookingStatus) {
          bookingStatus.textContent = 'Something went wrong sending your request. Please use WhatsApp below instead.';
          bookingStatus.className = 'form-status is-error';
        }
      } finally {
        if (submitBtn) { submitBtn.disabled = false; submitBtn.querySelector('.btn-label').textContent = 'Send Booking Request'; }
      }
    });
  }
